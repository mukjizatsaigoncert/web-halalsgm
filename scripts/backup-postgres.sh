...
#
# Env:
#   BACKUP_DIR       (default /var/backups/halal)
#   RETENTION_DAYS   (default 30)
#   BACKUP_S3_BUCKET (optional, requires aws CLI)
set -euo pipefail

BACKUP_DIR="${BACKUP_DIR:-/var/backups/halal}"
RETENTION_DAYS="${RETENTION_DAYS:-30}"
STAMP=$(date -u +%Y%m%dT%H%M%SZ)
FILE="${BACKUP_DIR}/halal-${STAMP}.sql.gz"
...
docker compose exec -T postgres pg_dump \
  --username="${POSTGRES_USER:-halal}" \
  --dbname="${POSTGRES_DB:-halal}" \
...
find "$BACKUP_DIR" -type f -name 'halal-*.sql.gz' -mtime +"$RETENTION_DAYS" -delete
